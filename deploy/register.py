...
def write_mysql_files(mount: Path, overwrite: bool) -> None:
    mount.mkdir(parents=True, exist_ok=True)
    try:
        os.chmod(mount, 0o700)
    except OSError:
    ...
    targets_path = mount / "mysql-targets.json"
    pass_path = mount / "mysqlpass"

    name = env_get("MYSQL_TARGET_NAME", "app") or "app"
    host = env_get("MYSQL_HOST", "host.docker.internal") or "host.docker.internal"
    port = int(env_get("MYSQL_PORT", "3306") or "3306")
    dbname = env_get("MYSQL_DBNAME", "yourdb") or "yourdb"
    user = env_get("MYSQL_USER", "mcp_ro") or "mcp_ro"
    sslmode = env_get("MYSQL_SSLMODE", "disable") or "disable"
    ...
    if not password:
        if not pass_path.is_file():
            print("MYSQL_PASSWORD empty; skip mysqlpass (create it before registering mysql)", file=sys.stderr)
        return

    if overwrite or not pass_path.is_file():
        line = f"{host}:{port}:{dbname}:{user}:{password}\n"
        pass_path.write_text(line, encoding="utf-8")
        os.chmod(pass_path, stat.S_IRUSR | stat.S_IWUSR)
        print(f"wrote {pass_path} (mode 600)", file=sys.stderr)
...
def register() -> int:
    ...
    if truthy(env_get("REGISTER_MYSQL", "false")):
        mysqlpass = disc.mount / "mysqlpass"
        mysql_targets = disc.mount / "mysql-targets.json"
        if not mysql_targets.is_file() or not mysqlpass.is_file():
            skip("REGISTER_MYSQL but mysql-targets.json or mysqlpass is missing on the mount")
        else:
            client.upsert(
                "mysql",
                stdio_config(
                    f"{bin_dir}/mysql-mcp-server",
                    None,
                    {
                        "MYSQL_TARGETS_FILE": f"{bin_dir}/mysql-targets.json",
                        "MYSQL_PASSFILE": f"{bin_dir}/mysqlpass",
                        "MYSQL_MCP_READ_ONLY": "true",
                    },
                    "DevOpsMCP mysql (stdio)",
                ),
                existing,
            )

    if truthy(env_get("REGISTER_HOST_LOGS", "false")):
        logs_targets = disc.mount / "host-logs-targets.json"
        if not logs_targets.is_file():
            skip("REGISTER_HOST_LOGS but host-logs-targets.json is missing on the mount")
        else:
            client.upsert(
                "host-logs",
                stdio_config(
                    f"{bin_dir}/host-logs-mcp-server",
                    None,
                    {
                        "HOST_LOGS_TARGETS_FILE": f"{bin_dir}/host-logs-targets.json",
                        "HOST_LOGS_READ_ONLY": "true",
                    },
                    "DevOpsMCP host-logs (stdio)",
                ),
                existing,
            )
...
def main() -> int:
    ...
    parser.add_argument("--write-mysql-files", action="store_true")
    parser.add_argument("--register", action="store_true")
    parser.add_argument("--overwrite-pg", action="store_true")
    parser.add_argument("--overwrite-mysql", action="store_true")
    ...
    if args.write_pg_files:
        ...
        return 0
    if args.write_mysql_files:
        disc = Discovery()
        write_mysql_files(disc.mount, overwrite=args.overwrite_mysql or truthy(env_get("OVERWRITE_MYSQL_CONFIG")))
        return 0
    if args.register:
        return register()
