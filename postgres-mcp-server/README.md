...
1. 将 Linux 二进制放入月弦可执行的位置。默认 Docker 挂载 `/data/ylune-mcp` → `/opt/mcp`，这里只需放程序，不用放凭据文件。新建 STDIO 服务器，命令 `/opt/mcp/postgres-mcp-server`，参数留空。平台原生部署则填实际二进制绝对路径。
2. 在「凭据中心」新建一条凭据，填写下面两个键，绑定该服务器：

| 凭据键 | 填写内容 |
| --- | --- |
| `PG_TARGETS_JSON` | 下方完整 JSON 文本，不是文件路径，也不用加外层引号 |
| `PG_MCP_READ_ONLY` | `true` |
...
- 未设置 `PG_TARGETS_JSON` 时，沿用 `PG_TARGETS_FILE` 和原有文件配置。
- 只要设置了 `PG_TARGETS_JSON`（即使为空），就优先采用平台模式；空值、格式错误、缺少必要字段均报错，不回退到文件。平台服务器无需再声明 `PG_TARGETS_FILE`。
