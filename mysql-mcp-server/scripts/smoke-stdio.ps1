# 作用：对本机编译的 mysql-mcp-server 做 stdio 握手，核对接手和只读工具清单
# 运行主机：本机
# 调用方：人工验收
# 大概流程：
#   1) 用临时 targets 文件拉起 stdio（不连真实 MySQL）
#   2) initialize → tools/list
#   3) 断言 21 个只读工具在，写工具不在
# 勿放密钥：targets 里不放密码

param(
    [string]$Binary = (Join-Path $PSScriptRoot "..\mysql-mcp-server.exe")
)

$ErrorActionPreference = "Stop"
$Binary = (Resolve-Path $Binary).Path

$targets = Join-Path $env:TEMP "mysql-mcp-smoke-targets.json"
$targetsJson = @'
{
  "targets": [
    {
      ...
      "user": "mcp_ro",
      "sslmode": "disable",
      "credential_ref": "mysql/smoke-local",
      "tags": ["smoke"]
    }
  ]
}
...
if ($psi.Environment) {
    $psi.Environment["MYSQL_TARGETS_FILE"] = $targets
    $psi.Environment["MYSQL_MCP_READ_ONLY"] = "true"
}
$psi.EnvironmentVariables["MYSQL_TARGETS_FILE"] = $targets
$psi.EnvironmentVariables["MYSQL_MCP_READ_ONLY"] = "true"
...
try {
    Send-Mcp '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"mysql-smoke","version":"0.1.0"}}}'
    ...
    $required = @(
        ...
        "get_server_overview", "get_database_stats", "get_settings", "get_status", "get_error_log",
        ...
        "get_innodb_metrics", "get_replication_status",
        "query_mysql", "explain_query"
    )
    $forbidden = @(
        "execute_sql", "run_sql", "write_query",
        "update_row", "delete_row", "insert_row",
        "kill_session", "kill_query", "flush_hosts",
        "create_index", "alter_table", "optimize_table"
    )
