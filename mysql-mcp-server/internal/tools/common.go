...
func (d Deps) query(ctx context.Context, target, sql string, args []any, maxRows int) (*mysqldb.Result, targets.Target, error) {
...
func wrapRows(target targets.Target, note string, res *mysqldb.Result) map[string]any {
	out := map[string]any{
		"target":    target.Name,
...
func putSection(out map[string]any, name string, res *mysqldb.Result) {
	out[name] = res.Rows
	if res.Truncated {
		out[name+"_truncated"] = true
		out[name+"_truncated_reason"] = res.TruncatedReason
	}
}

func optionalSection(ctx context.Context, d Deps, t targets.Target, sql string, args []any, maxRows int) (*mysqldb.Result, string) {
	res, err := d.Pool.Query(ctx, t, sql, args, maxRows)
	if err != nil {
		return &mysqldb.Result{Rows: []map[string]any{}}, err.Error()
	}
	return res, ""
}

func missingHint(err error, what, hint string) map[string]any {
	out := map[string]any{
		"error":  what,
		"hint":   hint,
		"detail": "",
