...
func TestPlatformCredentialsWithoutFiles(t *testing.T) {
	t.Setenv("MYSQL_TARGETS_JSON", `{"targets":[{"name":"test","host":"db.example.com","dbname":"demo","user":"reader","password":"test-secret"}]}`)
...
func TestPlatformErrorDoesNotExposeSecret(t *testing.T) {
	t.Setenv("MYSQL_TARGETS_JSON", `{"targets":[{"name":"SECRET_MARKER"}]}`)
	_, err := New("")
