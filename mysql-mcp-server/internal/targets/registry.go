...
func New(path string) (*Registry, error) {
	inline, platform := os.LookupEnv("MYSQL_TARGETS_JSON")
	if path == "" && !platform {
		return nil, fmt.Errorf("MYSQL_TARGETS_FILE or MYSQL_TARGETS_JSON is required")
	}
	r := &Registry{path: path, platformJSON: inline, platform: platform}
	if err := r.loadLocked(true); err != nil {
		if platform {
			return nil, fmt.Errorf("invalid MYSQL_TARGETS_JSON configuration")
		}
		return nil, err
	}
	return r, nil
...
var allowedSSLModes = map[string]struct{}{
	...
	"verify-ca": {}, "verify-full": {}, "skip-verify": {},
}
...
func (r *Registry) loadLocked(requireOK bool) error {
	...
	for i, t := range file.Targets {
		...
		if !r.platform && t.Password != "" {
			return fmt.Errorf("target %s must not store password in the targets file; use credential_ref or mysqlpass", name)
		}
		...
		if file.Targets[i].IsProduction() {
			switch mode {
			case "disable", "prefer", "skip-verify":
