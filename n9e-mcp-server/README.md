...
启动命令：`/opt/mcp/n9e-mcp-server`；参数：`stdio`。

在凭据中心新建例如 `nightingale-uat`，填写：

| 键 | 值示例 / 含义 |
| --- | --- |
| `N9E_BASE_URL` | `https://nightingale.example.com`，夜莺根地址 |
| `N9E_TOKEN` | 夜莺生成的只读访问 Token，直接在凭据中心填写真实值 |
| `N9E_READ_ONLY` | `true` |
| `N9E_TOOLSETS` | `alerts,targets,datasource,busi_groups,metrics,logs` |

这里的 `N9E_TOKEN` 是夜莺 Token，不是月弦 Access Key。不需要 targets 文件。连接后执行一个告警列表查询，确认 Token 权限及网络可用。
